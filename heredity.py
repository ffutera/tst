...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for key, value in people.items():
        ...
        if value["name"] in one_gene:
            ...
            if value["name"] in have_trait:
                chance *= PROBS["trait"][1][True]
            else:
                chance *= PROBS["trait"][1][False]

        elif value["name"] in two_genes:
            if mother is None and father is None:
                chance = PROBS["gene"][2]
            else:
                if mother_gene_count == 0 and father_gene_count == 2 or mother_gene_count == 2 and father_gene_count == 0:
...
def normalize(probabilities):
    ...
    for person in probabilities:
        trait_probability_true = probabilities[person]["trait"][True]
        trait_probability_false = probabilities[person]["trait"][False]
        trait_total = trait_probability_true + trait_probability_false
        trait_normalised_true = trait_probability_true/trait_total
        trait_normalised_false = trait_probability_false/trait_total
        probabilities[person]["trait"][True] = trait_normalised_true
        probabilities[person]["trait"][False] = trait_normalised_false

        gene_probability_zero = probabilities[person]["gene"][0]
        gene_probability_one = probabilities[person]["gene"][1]
        gene_probability_two = probabilities[person]["gene"][2]
        gene_total = gene_probability_zero + gene_probability_one + gene_probability_two
        gene_normalised_zero = gene_probability_zero / gene_total
        gene_normalised_one = gene_probability_one / gene_total
        gene_normalised_two = gene_probability_two / gene_total
        probabilities[person]["gene"][0] = gene_normalised_zero
        probabilities[person]["gene"][1] = gene_normalised_one
        probabilities[person]["gene"][2] = gene_normalised_two
